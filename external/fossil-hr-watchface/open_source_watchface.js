return {
    node_name: '',
    manifest: {
        timers: ['update_partial', 'update_full', 'hands', 'flick_away', 'menu', 'request_menu_data']
    },
    persist: {
        version: 1,
        data: ['menu_structure']
    ...
    handle_global_event: function (self, state_machine, event, response) {
        ...
        // Did the watch get connected to the app just now?
        if ((event.type ==='common_update') && (event.app_status === true) && (get_common().app_status === 'connected')) {
            // If the custom menu is undefined, start a timer to request for the menu. If we request it without delay, it does not go through
            if (Object.keys(self.menu_structure).length === 0) {
                start_timer(self.node_name, 'request_menu_data', 500);
            }
        }
    },
            ...
            case 'watch': {
                ...
                if (state_phase == 'during') {
                    return function (self, state_machine, event, response) {
                        ...
                        } else if ((event.type == 'timer_expired') && (is_this_timer_expired(event, self.node_name, 'request_menu_data'))) {
                            self.init_menu_data();
